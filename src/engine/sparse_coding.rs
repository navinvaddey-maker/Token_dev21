...
static HEDGE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(maybe|perhaps|possibly|sort of|kind of|i guess|i think|i believe)\b").unwrap()
});
...
static FILLER_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(basically|actually|just|really|very|quite|so|well|literally|essentially)\b").unwrap()
});
...
static ARTICLE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(the|a|an) (following|above|below|given)\b").unwrap()
});
...
static DUP_SEP_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"---\s*---(\s*---)*").unwrap()
});
static QUOTE_NOISE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#""\."\.""#).unwrap()
});
...
pub fn run(text: &str, mode: &str) -> PrincipleResult {
    ...
    let patterns: &[&Lazy<Regex>] = match mode {
        ...
        "aggressive" => &[&HEDGE_RE, &COURTESY_RE, &FILLER_RE, &META_RE, &PASSIVE_RE, &ARTICLE_RE, &SELF_REF_RE],
